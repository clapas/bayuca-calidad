...
$this->title = Yii::t('app', 'Guest sources');
$this->params['breadcrumbs'][] = $this->title;
?>
<div class="guest-sources">
...
$script = <<<JS
  ...
  var sources_labels = sources.map(function(v, k) {
      return v.source_title;
  });
  backgroundColors = [];
  sources.forEach(function(e, i) {
      backgroundColors.push(rainbow(sources.length, i));
  });
  var ctx = $('canvas');
  var data = {
      ...
      datasets: [{
          ...
          borderColor: '#ccc',
          borderWidth: 1,
          data: sources.map(function(v, k) {
              return v.count;
          })
      }]
  };
  function updateChart(data) {
      chart.config.data.labels = data.sources.map(function(v, k) { return v.source_title; });
      chart.config.data.datasets[0].data = data.sources.map(function(v, k) {
          return v.count;
      });
